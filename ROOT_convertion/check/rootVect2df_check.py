...
def calc_e(row):
    tot_e = 0
    for i in range(Npfc):    tot_e += row['E_'+str(i)]
    return tot_e
...
def run_nn_i(inName):
    ...
    cut_attr_lst = ['pt','eta','isGenMatched'    ,'cHadEFrac','energy']#'nHadEFrac','cHadMulti','nHadMulti','npr']
    ...
    """
    dfe = df_s[pre_str+'.'+'energy']
    dfj = df_s[pre_str+'.'+'jetIndex']
    dfm = df_s[cut_pre_str+'.'+'isGenMatched']
    for i in range(dfe.shape[0]):
        et   = 0
        lste = dfe[pre_str+'.'+'energy'][i]
        lstj = dfj[pre_str+'.'+'jetIndex'][i]
        cm   = dfm[cut_pre_str+'.'+'isGenMatched'][i]
    ...
    #print df_dict[pre_str+'.'+'energy']
    #print df_dict[pre_str+'.energy'].sum(axis=1)
    #exit()

    #for i in range(len(df)):
    #    print df[pre_str+'.'+'energy'][i+1]
    #for i in range(len(df_dict[pre_str+'.'+'energy'])):
    #    rr = next(df_dict[pre_str+'.'+'energy'].iterrows())
    #    print rr.count(axis='columns')
    #zz =  df_dict[pre_str+'.'+'energy'].count(axis='columns')
    #for i in range(2000):
    ...
    pan['E']              = pan.apply(calc_e, axis=1) 
    yyy                   = df[cut_pre_str+'.'+'energy'].reset_index()  
    pan['energy']         = yyy[cut_pre_str+'.'+'energy']
